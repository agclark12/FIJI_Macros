...
ch1_keyword = "Trans_reg"; //ch1 is the one used for registration
ch2_keyword = "mCherry_reg";
start_position = 1;
end_position = 10;

for (i=start_position; i<end_position+1; i++) {
	print("cropping position: s"+i);
	...
	print("autocropping black edges");
	run("Autocrop Black Edges");

	//split the channels up again
	selectWindow("comb_autocrop");
	/*	Stack.getDimensions(w, h, ch, no_slices, no_frames);
	...
	*/
	...
	//saves the images
	selectWindow("C1-reg");
	saveAs("tiff",save_dir + ch1_keyword + "_crop_s" + i + ".tif");
	selectWindow("C2-reg");
	saveAs("tiff",save_dir + ch2_keyword + "_crop_s" + i + ".tif");
	run("Close All");
	run("Collect Garbage");

...
for (i=0; i<lengthOf(dir_list); i++) {
	...
	if (matches(dir_list[i],".*" + keyword + ".*")) {
		...
		print("Opening Image");
		basename = substring(dir_list[i],0,indexOf(dir_list[i],".czi"));
		...
		*/
		...
		for (k=1;k<channels;k++) {
			run("Combine...", "stack1=[Combined Stacks] stack2=C"+(k+1)+"-stk");
		}
		...
		w = getWidth();
		h = getHeight();
		makeRectangle(w-160, h-40, 10, 10);
		...
		saveAs("tiff", save_dir + basename + "_bpp_w" + sb_size +"umSB_merge.tif");

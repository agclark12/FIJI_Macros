...
for (i=0; i<lengthOf(dir_list); i++) {
	...
	if (matches(dir_list[i],".*" + keyword + ".*")) {
		...
		//open(file_path);
		run("Bio-Formats", "open="+file_path+" autoscale color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
		rename("stk");
		...
		run("Merge Channels...", "c2=C3-stk c3=C1-stk c6=C4-stk create keep ignore");
		rename("merge");
		...
		w = getWidth();
		h = getHeight();
		makeRectangle(w-200, h-30, 10, 10);
		run("Scale Bar...", "width=" + sb_size + " height=12 font=14 color=White background=None location=[At Selection] bold hide label");
